...
static s32int find_smallest_hole(u32int size, u8int page_align, heap_t *heap) {
    ...
    while (iterator < heap->index.size) {
        header_t *header = (header_t *) lookup_ordered_array(iterator,
                &heap->index);
...
heap_t *create_heap(u32int start, u32int end_addr, u32int max, u8int supervisor,
        u8int readonly) {
...
void *halloc(u32int size, u8int page_align, heap_t *heap) {
    ...
    if (iterator == -1) // If we didn't find a suitable hole
        ...
        if (idx == -1) {
            ...
            header->size += new_length - old_length;
            // Rewrite the footer.
            footer_t *footer = (footer_t *) ((u32int) header + header->size
                    - sizeof(footer_t) );
            footer->header = header;
            footer->magic = HEAP_MAGIC;
        }
    ...
    header_t *orig_hole_header = (header_t *) lookup_ordered_array(iterator,
            &heap->index);
    ...
    if (page_align && orig_hole_pos & 0xFFFFF000) {
        u32int new_location = orig_hole_pos + PAGE_SIZE
        ...
        footer_t *hole_footer = (footer_t *) ((u32int) new_location
                - sizeof(footer_t) );
    ...
    // ...And the footer
    footer_t *block_footer = (footer_t *) (orig_hole_pos + sizeof(header_t)
            + size);
    ...
    if (orig_hole_size - new_size > 0) {
        ...
        footer_t *hole_footer = (footer_t *) ((u32int) hole_header
...
void hfree(void *p, heap_t *heap) {
    ...
    if ((u32int) footer + sizeof(footer_t) == heap->end_address) {
        u32int old_length = heap->end_address - heap->start_address;
        u32int new_length = contract((u32int) header - heap->start_address,
                heap);
        // Check how big we will be after resizing.
        if (header->size - (old_length - new_length) > 0) {
            // We will still exist, so resize us.
            header->size -= old_length - new_length;
            footer = (footer_t*) ((u32int) header + header->size
                    - sizeof(footer_t) );
            footer->magic = HEAP_MAGIC;
            footer->header = header;
        } else {
