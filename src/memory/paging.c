...
void _logPage(page_t page, int i, int j) {
    log(
            L_TRACE, "%d:%d (0x%x): f:0x%x ( %s%s%s%s) r:0x%x", i, j, (i<<22)|(j<<12), page.frame, page.present ? "present " : "not-present ", page.accessed ? "accessed " : "", page.dirty ? "dirty " : "", page.rw ? "read-write " : "", page.unused);
}
...
page_t *get_page(u32int address, int make, page_directory_t *dir) {
    ...
    if (dir->tables[table_idx]) { // If this table is already assigned
        ...
        u32int tmp;
        dir->tables[table_idx] = (page_table_t*) kmalloc_ap(
                sizeof(page_table_t), &tmp);
        memset(dir->tables[table_idx], 0, PAGE_SIZE);
        dir->tablesPhysical[table_idx] = tmp
                | (PAGE_PRESENT | PAGE_READWRITE | PAGE_USERMODE);
        return &dir->tables[table_idx]->pages[address % PAGE_COUNT];
    } else {
        return 0;
    }
...
void page_fault(registers_t regs) {
    ...
    log(
            L_ERROR, "PAGE-FAULT ( %s%s%s%s%s) at 0x%x", regs.err_code & PAGE_PRESENT ? "page-present " : "page-not-present ", regs.err_code & PAGE_READWRITE ? "write-on-read-only " : "", regs.err_code & PAGE_USERMODE ? "processor-was-in-user-mode " : "", regs.err_code & PAGE_CORRUPTED ? "overwritten-cpu-reserved-bits-of-page-entry " : "", regs.err_code & PAGE_INST_FETCH ? "instruction-fetch " : "", faulting_address);
    tty_setFormatToCurrTTY(video_getFormattedColor(RED, BLACK));
    _logPage(*get_page(faulting_address, 0, current_directory),
            faulting_address >> 22, faulting_address >> 12);
...
PRIVATE u32int create_stack(void *new_stack_start, u32int size) {
    ...
    for (i = (u32int) new_stack_start;
            i >= ((u32int) new_stack_start - size - 1); i -= PAGE_SIZE) {
