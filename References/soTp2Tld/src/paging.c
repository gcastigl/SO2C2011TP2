...
page_t *get_page( int address, int make, page_dir_t *dir );
...
void initPaging(void) 
{
   ...
   memset( kernel_directory, 0, sizeof(page_dir_t) ); // Se inicializa en 0 el directorio de paginas
   ...
   //    current_directory = kernel_directory;
    ...
    // Allocate a liltle bit extra so the kernel heap can be initialised properly.
...
void enablePaging( page_dir_t* dir )
{
  int cr0; 
...
static int first_fframe( void )
{
  int i,j,toTest;
   ...
   // podria recorrer cuantos ints sean necesarios para representar todos los frames en bits
...
void flushPages	( int pid, int action )
{
	...
	page_t *page;
	PROCESST * process, *proc_parent;
	
	if( pid == 0 )
	  return;
	
	for(proc_idx=0; proc_idx<MAXPROCESS && processes[proc_idx].pid!=pid;proc_idx++ )
	  ; 
	...
	pages=processes[proc_idx].stacksize/0x1000; // cuantas paginas tiene ese proceso

	//direccion de memoria donde comienza el stack ( operacion inversa de create process )
	mem_dir=processes[proc_idx].stackstart-processes[proc_idx].stacksize+1;
	...
	if( process->parent > 1 )
	{
		proc_parent=getProcessByPID( process->parent );
		flushPages( process->parent, action );
	}
