...
int  kmalloc	( int size );		//   General allocation function. 
void kfree	( int p );		//   General deallocation function.

void free 	( void *p, heap_t *heap); 		// Releases a block allocated with 'alloc'.
int kmalloc_a	( int size );			  	// malloc con alineacion a pagina de 4KB
int kmalloc_p	( int size, int *phys );		// malloc con retorno de direccion fisica
int kmalloc_ap	( int size, int *phys ); 		// malloc con alineacion a pagina & retorno de direccion fisica
